...
export default async function StatisticsPage() {
  ...
  const [views7, views30, daily, topPaths, byLocale, byCountry] = await Promise.all([
    ...
    <div className="mx-auto max-w-4xl px-8 py-10">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <Card>
          ...
          <p className="mt-1 font-display text-3xl font-semibold text-brand-ink">{views7}</p>
        </Card>
        <Card>
          <p className="font-mono text-xs uppercase tracking-widest text-ink-faint">{t("last30days")}</p>
          <p className="mt-1 font-display text-3xl font-semibold text-brand-ink">{views30}</p>
        </Card>
      </div>
